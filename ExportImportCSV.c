...
void savetocsvjadwal(jadwal_t j[][5]) {
    /*Membuat file agar hasil dapat dimasukkan ke dalam file csv*/
    FILE *outfile;
    outfile = fopen("Jadwal_Praktikum_Semester_2.csv","w");
    ...
    for (int minggu=0; minggu<12; minggu++) {
...
void savetocsvasisten(jadwal_t j[][5]) {
    /*Membuat file agar hasil dapat dimasukkan ke dalam file csv*/
    FILE *outfile;
    outfile = fopen("Jadwal_Asisten_Semester_2.csv","w");
...
void importcsv(jadwal_t j[][5]) {
    ...
    //input user
    printf("Masukkan nama file, tambahkan .csv: "); 
    scanf("%s", file);
    printf("Masuk fileinput\n");
    FILE* inp;
    printf("file %s\n", file);
    inp = fopen(file,"r");
    if (inp == NULL) {
